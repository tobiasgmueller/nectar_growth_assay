# 2021 plate analysis
# natural high deltaline treatment
# 3/31/21
# tobias mueller
...
# "edited" file below is an adjusted csv from the first run of deltaline treatment
# due to condensation there was a slight stop in OD for first ~30 minutes
# to account, we set starting OD to lowest value of first 45 minutes

# this was done in excel and both csv files can be found in input folder
# with deltaline022(...)cleaned.csv bring the original data
# and deltaline022(...)edited being the adjusted data

d <- read.csv("input/deltaline022_mar13_21_edited.csv")

#we reran a new plate of deltaline but none of the controls grew making those results
# questionable 
# do not use this plate
# d <- read.csv("input/deltaline022_mar31_21_cleaned.csv")
...
gr <- cbind(gr, treatment="high deltaline", date="mar 24")
...
write.csv(gr, "input/deltaline022_mar13_21_formatted.csv", row.names = FALSE)
...
ggplot(graph, aes(x=time, y=value_adjust, color=treatment)) +
  ...
  theme_bw() +
  labs(title="high deltaline adjusted")
...
gr<-read.csv("input/deltaline022_mar31_21_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/deltaline022_mar31_21_formatted.csv", header=F)
...
# decline 14, 38, 67, 87 if using edited csv


# declined 25, 26, 28, 29, 34 , 37 39, 40 if second run
...
write.csv(parms, "output/high_deltaline_summary")
