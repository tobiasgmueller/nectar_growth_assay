...
#read in data 
d <- read.csv("input/linalool_100ng_24march_21_secondread_cleaned.csv")
...
gr <- cbind(gr, treatment="100ng linalool", date="mar 24")
...
write.csv(gr, "input/linalool_100ng_24march_21_formatted.csv", row.names = FALSE)
...
gr<-read.csv("input/linalool_100ng_24march_21_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/linalool_100ng_24march_21_formatted.csv", header=F)
...
# declined wells 2, 12 , 16, 79

# well 80 is unusual - probably contamination but I dont have proof of that so we keep(?)

parms<-summary.gcFit(growth.test)
#use the built in summary function and write parameters to a df
...
write.csv(parms, "output/100ng_linalool_summary.csv")
