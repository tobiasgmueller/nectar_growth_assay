...
d <- read.csv("input/1_etoh_cleaned.csv")
...
gr <- cbind(gr, treatment="1% etoh", date="apr 19")
...
write.csv(gr, "input/1perc_etoh_formatted.csv", row.names = FALSE)
...
gr<-read.csv("input/1perc_etoh_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/1perc_etoh_formatted.csv", header=F)
...
# declined wells 20, 22,  56, 67, 91
# well 56 growth is due to contamination
...
write.csv(parms, "output/1perc_etoh_summary.csv")
