...
d <- read.csv("input/lowsugar_feb11_2021_cleaned.csv")
...
gr <- cbind(gr, treatment="2x sugar", date="feb 11")
...
write.csv(gr, "input/lowsugar_formatted.csv", row.names = FALSE)
...
gr<-read.csv("input/lowsugar_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/lowsugar_formatted.csv", header=F)
times<-t[1,5:ncol(t)]
...
write.csv(parms, "output/low_sugar_summary.csv")
...
#plot mu
p1 <- ggplot(parms_medsug, aes(x=treatment, y=mu.model, color=treatment)) +
...
#plot A
p2 <- ggplot(parms_medsug, aes(x=treatment, y=A.model, color=treatment)) +
  geom_boxplot(aes(), alpha=0.5) +
  geom_point(aes(), size=.5) +
  theme(axis.text.x=element_text(angle = -70, hjust = 0))+
...
#plot lambda
p3 <- ggplot(parms_medsug, aes(x=treatment, y=lambda.model, color=treatment)) +
  geom_boxplot(aes(), alpha=0.5) +
  geom_point(aes(), size=.5) +
  theme(axis.text.x=element_text(angle = -70, hjust = 0))+
