...
d <- read.csv("input/10x_aa_feb15_2021_cleaned.csv")
...
gr <- cbind(gr, treatment="10x aa", date="feb 15")
...
#plot mu
p1 <- ggplot(parms_highaa, aes(x=treatment, y=mu.model, color=treatment)) +
...
#plot A
p2 <- ggplot(parms_highaa, aes(x=treatment, y=A.model, color=treatment)) +
  geom_boxplot(aes(), alpha=0.5) +
  geom_point(aes(), size=.5) +
  theme(axis.text.x=element_text(angle = -70, hjust = 0))+
...
#plot lambda
p3 <- ggplot(parms_highaa, aes(x=treatment, y=lambda.model, color=treatment)) +
  geom_boxplot(aes(), alpha=0.5) +
  geom_point(aes(), size=.5) +
  theme(axis.text.x=element_text(angle = -70, hjust = 0))+
