#I have restructured this so it only pertains to ec102 run on 11 june 2020
# tobias mueller
#begin: july 29 2020
...
#lets try it with the no control data files
 d <- read.csv("input/nectarchem_ec102_11jun2020_tm.csv")
 d <- d %>% select(-A7, -B7, -C7, -D1, -D2,
                           -D3, -D4, -D5, -D7, -D8,
                           -D9, -D10, -D11, -D12,
                           -E6,-F6, -G6, -H6)
labels <- read.csv("input/labels_nocontrol.csv")

# fix labels classes
labels$treatment <- as.factor(labels$treatment)
labels$well <- as.factor(labels$well)
# get rid of replicate column - not sure why thats there..
labels<- labels[,-3]
...
gr <- cbind(gr, microbe="ec102", date="june 11")
...
write.csv(gr, "input/reformatted_ec102.csv", row.names = FALSE)
...
gr<-read.csv("input/reformatted_ec102.csv", na.strings=c(".", "NA"))
t<-read.csv("input/reformatted_ec102.csv", header=F)
times<-t[1,5:ncol(t)]
...
write.csv(parms, "outputs/ec102grofit_summary.csv")
...
ggsave("outputs/ec102plot.png", g, height = 8, width = 6, dpi = 600)
