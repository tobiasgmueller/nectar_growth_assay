# this only pertains to q1f2 run on 23 june 2020
# tobias mueller
...
gr <- cbind(gr, microbe="q1f2", date="june 23")
...
write.csv(gr, "input/reformatted_q1f2.csv", row.names = FALSE)
...
gr<-read.csv("input/reformatted_q1f2.csv", na.strings=c(".", "NA"))
t<-read.csv("input/reformatted_q1f2.csv", header=F)
times<-t[1,5:ncol(t)]
...
write.csv(parms, "outputs/q1f2grofit_summary.csv")
