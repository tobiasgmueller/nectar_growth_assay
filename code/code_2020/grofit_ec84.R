# this only pertains to ec84 run on 29 june 2020
# tobias mueller
...
gr <- cbind(gr, microbe="ec84", date="june 29")
...
write.csv(gr, "input/reformatted_ec84.csv", row.names = FALSE)
...
gr<-read.csv("input/reformatted_ec84.csv", na.strings=c(".", "NA"))
t<-read.csv("input/reformatted_ec84.csv", header=F)
times<-t[1,5:ncol(t)]
...
write.csv(parms, "outputs/ec84grofit_summary.csv")
# ---------------------- plots -------------------
...
ggsave("outputs/ec84plot.png", g, height = 20, width = 10, dpi = 800)
