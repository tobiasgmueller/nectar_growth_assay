...
#read in data 
d <- read.csv("input/2xsugar_13apr_21_cleaned.csv")
...
gr <- cbind(gr, treatment="2x sugar", date="apr 15")
...
write.csv(gr, "input/2xsugar_formatted.csv", row.names = FALSE)
...
#graph every signle growth curve
...
gr<-read.csv("input/2xsugar_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/2xsugar_formatted.csv", header=F)
...
write.csv(parms, "output/2xsugar_summary.csv")
