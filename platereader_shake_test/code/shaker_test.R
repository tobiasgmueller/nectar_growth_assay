...
setwd("C:/Users/obiew/Desktop/vannette/platereader_shake_test")
...
d <- read.csv("input/all_data_reads.csv")
labels <- read.csv("input/shaker_test_labels.csv")
...
gr <- cbind(gr, microbe="q1f2", date="feb 9")
...
write.csv(gr, "input/shake_test_formatted.csv", row.names = FALSE)
...
gr<-read.csv("input/shake_test_formatted.csv", na.strings=c(".", "NA"))
t<-read.csv("input/shake_test_formatted.csv", header=F)
times<-t[1,5:ncol(t)]
...
write.csv(parms, "output/shake_test_summary.csv")
...
#plot A
p2 <- ggplot(parms, aes(x=TestId, y=A.model)) +
  geom_boxplot(aes(), alpha=0.5) +
...
#plot lambda
p3 <- ggplot(parms, aes(x=TestId, y=lambda.model)) +
  geom_boxplot(aes(), alpha=0.5) +
